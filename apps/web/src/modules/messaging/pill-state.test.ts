 ...
 *
 * El copy sale ya del catálogo: se le inyecta el traductor REAL (el mismo
 * `createTranslator` de next-intl que usa la app), así que las aserciones
 * siguen guardando el español byte a byte y además prueban que el inglés no
 * degrada.
 */

const t = createTranslator({ locale: 'es-ES', messages: es, namespace: 'modMessaging' });
const tEn = createTranslator({ locale: 'en-US', messages: en, namespace: 'modMessaging' });
...
describe('derivePillState — caras', () => {
  ...
  it('nunca pinta más de tres ni repite persona', () => {
    ...
    const state = derivePillState({ conversations, typing: [], viewerId: VIEWER, now: NOW }, t);
...
describe('derivePillState — línea 2 y estado vacío', () => {
  ...
  it('en inglés no queda ni una palabra en español (MUST-FIX 19)', () => {
    const state = derivePillState(
      {
        conversations: [
          room({ id: 'a', unreadCount: 2, lastMessage: lastMessage('u1', '2026-07-30T10:00:00Z') }),
    ...
    expect(state.line1.prefix).toBe('2 new messages in');
    expect(state.line2).toBe('2 rooms · 2 unread');
  });

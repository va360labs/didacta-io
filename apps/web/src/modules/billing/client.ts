...
/** Resultado de sincronizar precios desde la tienda WooCommerce. */
export interface WooSyncReport {
...
/**
 * Trae de la tienda WooCommerce el precio de los cursos que se venden sueltos.
 * Los packs de varios cursos se ignoran: esos son la membresía, que se gobierna
 * desde Didacta. Idempotente.
 */
export async function syncWoocommercePrices(
  bearer: string,
  opts: { dryRun?: boolean } = {},
): Promise<WooSyncReport> {
  const qs = opts.dryRun ? '?dryRun=true' : '';
  return apiFetch<WooSyncReport>(
    `/api/v1/modules/billing/products/sync-woocommerce${qs}`,
    { method: 'POST' },
    bearer,
  );
}

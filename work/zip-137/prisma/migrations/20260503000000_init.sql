...
-- Job principal
-- ============================================================
CREATE TABLE IF NOT EXISTS mod_migrator_learndash_jobs (
...
CREATE INDEX IF NOT EXISTS mod_migrator_learndash_jobs_tenant_status_idx
  ON mod_migrator_learndash_jobs (tenant_id, status);
CREATE INDEX IF NOT EXISTS mod_migrator_learndash_jobs_tenant_creator_started_idx
  ON mod_migrator_learndash_jobs (tenant_id, created_by, started_at);

-- ============================================================
-- Mapeo source_id -> target_id
-- ============================================================
CREATE TABLE IF NOT EXISTS mod_migrator_learndash_mappings (
  id UUID NOT NULL DEFAULT gen_random_uuid(),
  tenant_id UUID NOT NULL,
...
CREATE UNIQUE INDEX IF NOT EXISTS mod_migrator_learndash_mappings_unique
  ON mod_migrator_learndash_mappings (tenant_id, job_id, entity_type, source_id);
CREATE INDEX IF NOT EXISTS mod_migrator_learndash_mappings_lookup_idx
  ON mod_migrator_learndash_mappings (tenant_id, entity_type, source_id);
CREATE INDEX IF NOT EXISTS mod_migrator_learndash_mappings_job_status_idx
  ON mod_migrator_learndash_mappings (tenant_id, job_id, status);
CREATE INDEX IF NOT EXISTS mod_migrator_learndash_mappings_external_idx
  ON mod_migrator_learndash_mappings (tenant_id, external_id);

-- ============================================================
-- DLQ
-- ============================================================
CREATE TABLE IF NOT EXISTS mod_migrator_learndash_dlq (
  id UUID NOT NULL DEFAULT gen_random_uuid(),
  tenant_id UUID NOT NULL,
  job_id UUID NOT NULL,
  entity_type TEXT NOT NULL,
...
CREATE INDEX IF NOT EXISTS mod_migrator_learndash_dlq_job_phase_idx
  ON mod_migrator_learndash_dlq (tenant_id, job_id, phase);
CREATE INDEX IF NOT EXISTS mod_migrator_learndash_dlq_error_idx
  ON mod_migrator_learndash_dlq (tenant_id, error_code);

-- ============================================================
-- Audit events (append-only, cadena SHA-256)
-- ============================================================
CREATE TABLE IF NOT EXISTS mod_migrator_learndash_audit_events (
  id UUID NOT NULL DEFAULT gen_random_uuid(),
  tenant_id UUID NOT NULL,
  job_id UUID NOT NULL,
  actor TEXT NOT NULL,
...
CREATE INDEX IF NOT EXISTS mod_migrator_learndash_audit_events_job_time_idx
  ON mod_migrator_learndash_audit_events (tenant_id, job_id, occurred_at);
CREATE INDEX IF NOT EXISTS mod_migrator_learndash_audit_events_action_idx
  ON mod_migrator_learndash_audit_events (tenant_id, action);

-- ============================================================
-- Validation reports
-- ============================================================
CREATE TABLE IF NOT EXISTS mod_migrator_learndash_validation_reports (
  id UUID NOT NULL DEFAULT gen_random_uuid(),
  tenant_id UUID NOT NULL,
  job_id UUID NOT NULL,
  entity_type TEXT NOT NULL,
...
CREATE UNIQUE INDEX IF NOT EXISTS mod_migrator_learndash_validation_reports_unique
...
CREATE TABLE IF NOT EXISTS mod_migrator_learndash_stg_users (
  id UUID NOT NULL DEFAULT gen_random_uuid(),
  tenant_id UUID NOT NULL,
  job_id UUID NOT NULL,
  source_id TEXT NOT NULL,
...
CREATE UNIQUE INDEX IF NOT EXISTS mod_migrator_learndash_stg_users_unique
  ON mod_migrator_learndash_stg_users (tenant_id, job_id, source_id);
CREATE INDEX IF NOT EXISTS mod_migrator_learndash_stg_users_valid_idx
  ON mod_migrator_learndash_stg_users (tenant_id, job_id, is_valid);

CREATE TABLE IF NOT EXISTS mod_migrator_learndash_stg_courses (
  id UUID NOT NULL DEFAULT gen_random_uuid(),
  tenant_id UUID NOT NULL,
  job_id UUID NOT NULL,
  source_id TEXT NOT NULL,
...
CREATE UNIQUE INDEX IF NOT EXISTS mod_migrator_learndash_stg_courses_unique
  ON mod_migrator_learndash_stg_courses (tenant_id, job_id, source_id);
CREATE INDEX IF NOT EXISTS mod_migrator_learndash_stg_courses_valid_idx
  ON mod_migrator_learndash_stg_courses (tenant_id, job_id, is_valid);

CREATE TABLE IF NOT EXISTS mod_migrator_learndash_stg_lessons (
  id UUID NOT NULL DEFAULT gen_random_uuid(),
  tenant_id UUID NOT NULL,
  job_id UUID NOT NULL,
  source_id TEXT NOT NULL,
...
CREATE UNIQUE INDEX IF NOT EXISTS mod_migrator_learndash_stg_lessons_unique
  ON mod_migrator_learndash_stg_lessons (tenant_id, job_id, source_id);
CREATE INDEX IF NOT EXISTS mod_migrator_learndash_stg_lessons_parent_idx
  ON mod_migrator_learndash_stg_lessons (tenant_id, job_id, parent_course_id, order_idx);

CREATE TABLE IF NOT EXISTS mod_migrator_learndash_stg_topics (
  id UUID NOT NULL DEFAULT gen_random_uuid(),
  tenant_id UUID NOT NULL,
  job_id UUID NOT NULL,
  source_id TEXT NOT NULL,
...
CREATE UNIQUE INDEX IF NOT EXISTS mod_migrator_learndash_stg_topics_unique
  ON mod_migrator_learndash_stg_topics (tenant_id, job_id, source_id);
CREATE INDEX IF NOT EXISTS mod_migrator_learndash_stg_topics_parent_idx
  ON mod_migrator_learndash_stg_topics (tenant_id, job_id, parent_lesson_id, order_idx);

CREATE TABLE IF NOT EXISTS mod_migrator_learndash_stg_quizzes (
  id UUID NOT NULL DEFAULT gen_random_uuid(),
  tenant_id UUID NOT NULL,
  job_id UUID NOT NULL,
  source_id TEXT NOT NULL,
...
CREATE UNIQUE INDEX IF NOT EXISTS mod_migrator_learndash_stg_quizzes_unique
  ON mod_migrator_learndash_stg_quizzes (tenant_id, job_id, source_id);
CREATE INDEX IF NOT EXISTS mod_migrator_learndash_stg_quizzes_job_idx
  ON mod_migrator_learndash_stg_quizzes (tenant_id, job_id);

CREATE TABLE IF NOT EXISTS mod_migrator_learndash_stg_questions (
  id UUID NOT NULL DEFAULT gen_random_uuid(),
  tenant_id UUID NOT NULL,
  job_id UUID NOT NULL,
  source_id TEXT NOT NULL,
...
CREATE UNIQUE INDEX IF NOT EXISTS mod_migrator_learndash_stg_questions_unique
  ON mod_migrator_learndash_stg_questions (tenant_id, job_id, source_id);
CREATE INDEX IF NOT EXISTS mod_migrator_learndash_stg_questions_quiz_idx
  ON mod_migrator_learndash_stg_questions (tenant_id, job_id, parent_quiz_id);

CREATE TABLE IF NOT EXISTS mod_migrator_learndash_stg_groups (
  id UUID NOT NULL DEFAULT gen_random_uuid(),
  tenant_id UUID NOT NULL,
  job_id UUID NOT NULL,
  source_id TEXT NOT NULL,
...
CREATE UNIQUE INDEX IF NOT EXISTS mod_migrator_learndash_stg_groups_unique
  ON mod_migrator_learndash_stg_groups (tenant_id, job_id, source_id);
CREATE INDEX IF NOT EXISTS mod_migrator_learndash_stg_groups_valid_idx
  ON mod_migrator_learndash_stg_groups (tenant_id, job_id, is_valid);

CREATE TABLE IF NOT EXISTS mod_migrator_learndash_stg_enrollments (
  id UUID NOT NULL DEFAULT gen_random_uuid(),
  tenant_id UUID NOT NULL,
  job_id UUID NOT NULL,
...
CREATE UNIQUE INDEX IF NOT EXISTS mod_migrator_learndash_stg_enrollments_unique
...
CREATE INDEX IF NOT EXISTS mod_migrator_learndash_stg_enrollments_user_idx
  ON mod_migrator_learndash_stg_enrollments (tenant_id, job_id, source_user_id);

CREATE TABLE IF NOT EXISTS mod_migrator_learndash_stg_progress (
  id UUID NOT NULL DEFAULT gen_random_uuid(),
  tenant_id UUID NOT NULL,
  job_id UUID NOT NULL,
  source_user_id TEXT NOT NULL,
...
CREATE UNIQUE INDEX IF NOT EXISTS mod_migrator_learndash_stg_progress_unique
  ON mod_migrator_learndash_stg_progress (tenant_id, job_id, source_user_id, source_course_id, source_step_id);
CREATE INDEX IF NOT EXISTS mod_migrator_learndash_stg_progress_user_idx
  ON mod_migrator_learndash_stg_progress (tenant_id, job_id, source_user_id);

CREATE TABLE IF NOT EXISTS mod_migrator_learndash_stg_media (
  id UUID NOT NULL DEFAULT gen_random_uuid(),
  tenant_id UUID NOT NULL,
  job_id UUID NOT NULL,
  source_id TEXT NOT NULL,
...
CREATE UNIQUE INDEX IF NOT EXISTS mod_migrator_learndash_stg_media_unique
  ON mod_migrator_learndash_stg_media (tenant_id, job_id, source_id);
CREATE INDEX IF NOT EXISTS mod_migrator_learndash_stg_media_valid_idx
